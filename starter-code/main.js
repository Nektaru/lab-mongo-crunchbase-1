      ...
      rl.question('Type an option: ', (option) => {
        switch (option) {
          ...
          case "5":
              ...
              db.collection('companies').find({$and: [{founded_year: 2004},{founded_month: {$gt: 4, $lt: 6}}]}, { name: 1, _id: 0 }).count((error, result) => {
                if (error) {
                  console.log(error);
                  rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                } else {
        ...
        case "7":
              ...
              db.collection('companies').find({"number_of_employees": 1}, {number_of_employees: -1}, { name: 1, _id: 0, number_of_employees: 1}).limit(10).count((error, result) => {
                if (error) {
                  console.log(error);
                  rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                } else {
        ...
        case "9":
          ...
          break;
        case "10":
           //10.- List the name of all the products of Facebook           Esto del findOne no sé como sale bien, me ha ayudado Guido.
